...
from electrum_btcc.util import profiler, parse_URI, format_time, InvalidPassword, NotEnoughFunds, Fiat
from electrum_btcc import bitcoin
from electrum_btcc.transaction import TxOutput
from electrum_btcc.util import timestamp_to_datetime
...
from electrum_btcc.gui.kivy.i18n import _
...
class CScreen(Factory.Screen):
    ...
    def load_screen(self):
        self.screen = Builder.load_file('electrum_btcc/gui/kivy/uix/ui_screens/' + self.kvname + '.kv')
...
class HistoryScreen(CScreen):
    ...
    def get_card(self, tx_hash, tx_mined_status, value, balance):
        ...
        icon = "atlas://electrum_btcc/gui/kivy/theming/light/" + TX_ICONS[status]
...
class SendScreen(CScreen):
    ...
    def set_URI(self, text):
        if not self.app.wallet:
            ...
            return
        import electrum_btcc as electrum
        try:
            uri = electrum.util.parse_URI(text, self.app.on_pr)
        except:
            self.app.show_info(_("Not a Bitcored URI"))
            return
    ...
    def do_save(self):
        ...
        from electrum_btcc.paymentrequest import make_unsigned_request, PaymentRequest
    ...
    def do_send(self):
        if self.screen.is_pr:
            ...
            if not address:
                self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a Bitcored address or a payment request'))
                return
            if not bitcoin.is_address(address):
                self.app.show_error(_('Invalid Bitcored Address') + ':\n' + address)
                return
            try:
...
class ReceiveScreen(CScreen):
    ...
    def get_URI(self):
        from electrum_btcc.util import create_URI
    ...
    def do_share(self):
        uri = self.get_URI()
        self.app.do_share(uri, _("Share Bitcored Request"))
