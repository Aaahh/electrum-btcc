...
_ = lambda x:x
#from i18n import _
from electrum_btcc import WalletStorage, Wallet
from electrum_btcc.util import format_satoshis, set_verbosity
from electrum_btcc.bitcoin import is_address, COIN, TYPE_ADDRESS
...
class ElectrumGui:
    ...
    def __init__(self, config, daemon, plugins):
        ...
        if not storage.file_exists:
            print("Wallet not found. try 'electrum-btcc create'")
            exit()
    ...
    def do_send(self):
        if not is_address(self.str_recipient):
            print(_('Invalid Bitcored address'))
            return
        try:
    ...
    def network_dialog(self):
        print("use 'electrum-btcc setconfig server/proxy' to change your network settings")
        return True


    def settings_dialog(self):
        print("use 'electrum-btcc setconfig' to change your settings")
        return True
