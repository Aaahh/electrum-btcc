...
class Bit2C(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('www.bit2c.co.il', '/Exchanges/BTCCNIS/Ticker.json')
...
class BitcoinAverage(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        return dict([(r.replace("BTCC", ""), Decimal(json[r]['last']))
    ...
    async def request_history(self, ccy):
        history = await self.get_csv('apiv2.bitcoinaverage.com',
                               "/indices/global/history/BTCC%s?period=alltime&format=csv" % ccy)
...
class BitcoinVenezuela(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        rates = [(r, json['BTCC'][r]) for r in json['BTCC']
                 if json['BTCC'][r] is not None]  # Giving NULL sometimes
        return dict(rates)
    ...
    async def request_history(self, ccy):
        json = await self.get_json('api.bitcoinvenezuela.com',
                             "/historical/index.php?coin=BTCC")
        return json[ccy +'_BTCC']


class Bitfinex(ExchangeBase):

    async def get_rates(self, ccy):
        json = await self.get_json('api.bitfinex.com', '/v1/pubticker/btccusd')
...
class Bitso(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.bitso.com', '/v3/ticker/?book=btcc_mxn')
...
class BitStamp(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('www.bitstamp.net', '/api/v2/ticker/btccusd/')
...
class Coinbase(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coinbase.com',
                                   '/v2/exchange-rates?currency=BTCC')
...
class CoinSpot(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        return {'AUD': Decimal(json['prices']['btcc']['last'])}
...
class GoCoin(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        btcc_prices = json['prices']['BTCC']
        return dict([(r, Decimal(btcc_prices[r])) for r in btcc_prices])
...
class HitBTC(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ccys = ['USD']
        json = await self.get_json('api.hitbtc.com', '/api/2/public/ticker/BTCC%s' % ccy)
...
class Kraken(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        pairs = [k for k in dicts['result'] if k.startswith('XBTCCZ')]
        json = await self.get_json('api.kraken.com',
                             '/0/public/Ticker?pair=%s' % ','.join(pairs))
        ccys = [p[5:] for p in pairs]
        result = dict.fromkeys(ccys)
        result[ccy] = Decimal(json['result']['XBTCCZ'+ccy]['c'][0])
        return result
    ...
    async def request_history(self, ccy):
        query = '/0/public/OHLC?pair=BTCC%s&interval=1440' % ccy
        json = await self.get_json('api.kraken.com', query)
        history = json['result']['XBTCCZ'+ccy]
...
class OKCoin(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('www.okcoin.com', '/api/v1/ticker.do?symbol=btcc_usd')
...
class MercadoBitcoin(ExchangeBase):
    ...
    async def get_rates(self,ccy):
        json = await self.get_json('www.mercadobitcoin.net', '/api/btcc/ticker/')
...
class TheRockTrading(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.therocktrading.com',
                                   '/v1/funds/BTCCEUR/ticker')
        return {'EUR': Decimal(json['last'])}

class QuadrigaCX(ExchangeBase):

    async def get_rates(self,ccy):
        json = await self.get_json('api.quadrigacx.com', '/v2/ticker?book=btcc_cad')
...
class WEX(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json_eur = await self.get_json('wex.nz', '/api/3/ticker/btcc_eur')
        json_rub = await self.get_json('wex.nz', '/api/3/ticker/btcc_rur')
        json_usd = await self.get_json('wex.nz', '/api/3/ticker/btcc_usd')
        return {'EUR': Decimal(json_eur['btcc_eur']['last']),
                'RUB': Decimal(json_rub['btcc_rur']['last']),
                'USD': Decimal(json_usd['btcc_usd']['last'])}
