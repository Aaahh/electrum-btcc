...
if [ -z "$1" ]; then
    echo "Usage: $0 Electrum-BTCC.app"
    exit -127
fi
...
if ! which ${genisoimage} > /dev/null 2>&1; then
	mkdir -p /tmp/electrum-btcc-macos
	cd /tmp/electrum-btcc-macos
...
rm -rf /tmp/electrum-btcc-macos/image > /dev/null 2>&1
mkdir /tmp/electrum-btcc-macos/image/
cp -r $1 /tmp/electrum-btcc-macos/image/
...
${genisoimage} \
    ...
    -dir-mode 0755 \
    -apple \
    -o Electrum-BTCC_uncompressed.dmg \
    /tmp/electrum-btcc-macos/image || fail "Unable to create uncompressed dmg"

dmg dmg Electrum-BTCC_uncompressed.dmg electrum-btcc-$VERSION.dmg || fail "Unable to create compressed dmg"
rm Electrum-BTCC_uncompressed.dmg

echo "Done."
md5sum electrum-btcc-$VERSION.dmg
