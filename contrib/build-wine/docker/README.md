...
2. Build image
    ...
    ```
    $ sudo docker build --no-cache -t electrum-btcc-wine-builder-img contrib/build-wine/docker
    ```
...
3. Build Windows binaries
    ...
    $ git checkout $REV
    $ sudo docker run \
        --name electrum-btcc-wine-builder-cont \
        -v $PWD:/opt/wine64/drive_c/electrum-btcc \
        --rm \
        --workdir /opt/wine64/drive_c/electrum-btcc/contrib/build-wine \
        electrum-btcc-wine-builder-img \
        ./build.sh
    ```
...
Electrum-BTCC Windows builds are signed with a Microsoft Authenticode™ code signing
certificate in addition to the GPG-based signatures.

The advantage of using Authenticode is that Electrum-BTCC users won't receive a 
...
|-----------------------------------------------------------------------------------------------|
| Upload signatures to 'electrum-btcc-signatures' repo, as `$version/$filename.$builder.asc`         |
