...
version=`python3 -c "import electrum_btcc; print(electrum_btcc.version.ELECTRUM_VERSION)"`
...
for mine in $(ls dist/*.exe); do
    echo "---------------"
    f=$(basename $mine)
    echo "Downloading https://electrum-btcc.org/download/$f"
    wget -q https://electrum-btcc.org/download/$f -O signed/$f
