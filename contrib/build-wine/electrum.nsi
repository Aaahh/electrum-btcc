...
;Variables
  ...
  !define PRODUCT_NAME "Electrum-BTCC"
  !define PRODUCT_WEB_SITE "https://github.com/aaahh/electrum-btcc"
...
;General
  ...
  ;Name and file
  Name "${PRODUCT_NAME}"
  OutFile "dist/electrum-btcc-setup.exe"
...
;Interface Settings
  ...
  !define MUI_ICON "c:\electrum-btcc\icons\electrum.ico"
...
Section
  ...
  File /r "dist\electrum-btcc\*.*"
  File "c:\electrum-btcc\icons\electrum.ico"
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-btcc-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-btcc-${PRODUCT_VERSION}.exe" "" "$INSTDIR\electrum-btcc-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\electrum-btcc-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\electrum-btcc-${PRODUCT_VERSION}.exe" 0


  ;Links bitcoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\bitcored" "" "URL:bitcored Protocol"
  WriteRegStr HKCU "Software\Classes\bitcored" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\bitcored" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\bitcored\shell\open\command" "" "$\"$INSTDIR\electrum-btcc-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\bitcored"
